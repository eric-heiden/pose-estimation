...
{
    typedef boost::variant<int, float, char, bool> SupportedValue;

    class ConsoleArgumentCategory;
    ...
    {
        friend class ConsoleArgumentCategory;
    public:
        ConsoleArgument(const std::string &category,
    ...
    private:
        ...
        static std::map<std::string, ConsoleArgument*> _allArgs;
        static std::map<std::string, std::vector<ConsoleArgument*> > _categorized;
    ...
    {
    public:
        ConsoleArgumentCategory(const std::string &name, const std::string &description = "");
    };
}
